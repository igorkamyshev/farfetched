import { attach, createEffect, Event, sample, withRegion } from 'effector';
...
export function cache<Q extends Query<any, any, any, any>>(
  ...
  return withRegion(
    createMetaNode({
      type: 'operator',
      operator: 'cache',
      target: query.__.meta.node,
      adapter: adapter.name,
    }),
    () => {
      const id = queryUniqId(query);

      const sourcedReaders =
        query.__.lowLevelAPI.sourced.map(createSourcedReader);
      ...
      if (purge) {
        sample({
          ...
          target: createEffect(
            ({ instance }: { instance: CacheAdapterInstance }) =>
              instance.purge()
          ),
        });
      }
